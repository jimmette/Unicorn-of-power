...
class Engine {
  ...
  */
  ...
  fireABullet() {
    //Only one bullet at a time on the board
    if (this.player.nbAmmo > 0 && this.bullet === undefined) {
  ...
  gameLoop() {
    ...
    if (this.bullet !== undefined) {
      // draw the bullet if bullet was fired
      this.bullet.render();
    }
  ...
  isBulletCollidedWithEnemy() {
    for (let i = 0; i < this.entities.length; i++) {
      if (
        ...
        this.entities[i] instanceof Enemy &&
        this.isCollision(this.bullet, this.entities[i])
      ) {
        this.entities[i].destroy();
        delete this.entities[i];
        this.bullet.destroy();
